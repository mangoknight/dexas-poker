...
//           });
      ...
      //  this.genzhunum.string="跟注"+255;//用这个来显示用户要跟注的筹码
        console.log('room'+self.room+'    '+self.userid+'++++++++++++++');
          ...
          self.scheduleOnce(function() {
              socket.emit('yule',{roomid:self.room,userid:self.userid,name:m.name,money:(JSON.parse(cc.sys.localStorage.getItem("roomid")).money)*0.98});
            }, 2);
        ...
        socket.on(self.room+'jieshu',function(data){
            data=JSON.parse(data);
             ...
             setTimeout(function() {
                ...
                self.dealer2.active=false;}, 4000);
            if(self.userid==data.id&&data.statusa==66){
                console.log('恭喜你获得了奖金'+data.num);
                self.audioMng.playWin();
                     console.log('在得奖这里获得了      allmoney1 '+self.allmoney);
                    self.allmoney+=data.num;
                    
                     console.log('在得奖这里获得了      allmoney2 '+self.allmoney);
                       var mmsg={roomid:self.room,name:m.name,num:self.allmoney,action:''};
                     socket.emit('changeMoney',mmsg);         
                     setTimeout(function(){ socket.emit('baocunmoney',{name:m.name,money:self.allmoney,type:self.type});},1000)
                    //   var xxx={name:m.name};
                    socket.emit('homeCoin',xxx);
        ...
        socket.on(self.userid+'one',function(data){
            ...
            self.allmoney-=fangfei;
            self.xiazhumoney=fangfei*0.98;
            var mmsg={roomid:self.room,name:m.name,num:self.allmoney,action:''};
            socket.emit('changeMoney',mmsg);
            setTimeout(function(){ socket.emit('baocunmoney',{name:m.name,money:self.allmoney,type:self.type});},1000)
           
                    //   socket.emit('xiazhu', { id: self.userid, roomid: self.room, money: fangfei*0.98, onepai: self.onefapai });

             // socket.emit('xiazhu',{id:self.userid,roomid:self.room,money:fangfei*0.98});
            self.p1.opacity = 255;
            self.p2.opacity = 255;
            self.p3.opacity = 255;
            self.p4.opacity = 255;
            ...
            self.id=data.uid;
              //localStorage.setItem("id",data.uid);
                if(data.name=="zhuang"){
                    ...
                    console.log("你是小盲注");
                    socket.emit('yulefapai',{roomid:self.room,userid:self.userid,name:m.name,money:(JSON.parse(cc.sys.localStorage.getItem("roomid")).money)*0.98});
  ...
  closeresult:function(){
       ...
       tuifang:function(){
        ...
        var self=this;
         ...
         socket.emit('tuifangqipai111',{id:self.userid,roomid:self.room});
